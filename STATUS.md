...
> **State legend (used throughout):** **Current** = already in the repo · **Active** = current main dev line · **Frozen** = done, maintenance only · **Target** = final architecture direction, **not yet implemented**. Do not treat `Target` items — Visual Design Decompiler, multi-layer IR (Evidence/Component/Editable/Correction), run manifest, component-level loop, audit-driven refinement tasks, multi-dimensional metrics — as existing code.
